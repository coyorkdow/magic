...
class BinarySearch {
  struct Less {
    static constexpr bool Result(const T& _l, const T& _r) { return Comp::Result(_l, _r); }
  };

  struct LessOrEqual {
    static constexpr bool Result(const T& _l, const T& _r) { return !Less::Result(_r, _l); }
  };
...
#define MID (l + ((r - l) >> 1))
  ...
  static constexpr size_t PerformSearch(T const (&array)[N], T value, size_t l, size_t r, Compare value_compare) {
    return (l == r)                           ? l
           : value_compare(array[MID], value) ? PerformSearch(array, value, MID + 1, r, value_compare)
