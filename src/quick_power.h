...
class Power {
  ...
  struct Process {
    ...
    static const uint64_t result =
        IfThenElse<bool(DIGIT& Exp), Times<Current, next::result>, Times<1, next::result>>::result::result;
  };
 ...
 public:
  inline constexpr static uint64_t Calculate() { return Process<1, Base>::result; }
};
